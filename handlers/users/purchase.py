...
async def show_items(message: Message):
    await message.answer(text="Добрый день! Сейчас нужно раз и навсегда решить! \n"
                              "Котики или песики?", reply_markup=choice)
...
async def buying_pear(call: CallbackQuery):
    ...
    await call.message.answer(f"Вы выбрали груши. Осталось еще много. Спасибо.",
...
async def buying_apples(call: CallbackQuery, callback_data: dict):
    ...
    await call.message.answer(f"Вы выбрали яблоки. Яблок осталось {quantity} шт. Спасибо.",
...
async def cancel_buying(call: CallbackQuery):
    ...
    await call.answer("Эй, так нечестно!", show_alert=True)
